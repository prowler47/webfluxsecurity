...
public class SecurityService {
    ...
    //TODO: find new way to create token without deprecated methods of Jwts
       ...
       return userService.getUserByUserName(name)
                .flatMap(user -> {
                    ...
                    if (!passwordEncoder.matches(password, user.getPassword())) {
                        return Mono.error(new AuthException("Invalid password", "DRAGUNOVSKIY_INVALID_PASSWORD"));
                    }
                    return Mono.just(generateToken(user).toBuilder()
                                    .userId(user.getId())
                            .build());
                })
                .switchIfEmpty(Mono.error(new AuthException("Invalid username", "DRAGUNOVSKIY_INVALID_USERNAME")));
    }
}
